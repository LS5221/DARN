...
from utils.data_utils import data_prep, DataSetCatCon_mlp, DatasetFLViT, create_dataset_and_evalmetrix, embed_data_mask
from utils.util import Partial_Client_Selection, valid, average_model, compute_weight
...
def train(args, model):
    ...
    X_train_dic, y_train_dic, X_valid_dic, y_valid_dic, X_test_dic, y_test_dic, train_mean_dic, train_std_dic, ips_dic, mask_dic = data_prep(
    ...
    while True:
        ...
        for cur_single_client in args.proxy_clients:
            ...
            train_ds = DataSetCatCon_mlp(X_train, y_train, args.cat_idxs, args.dtask, continuous_mean_std)
            ...
            valid_ds = DataSetCatCon_mlp(X_valid, y_valid, args.cat_idxs, args.dtask, continuous_mean_std)
            ...
            for inner_epoch in range(args.E_epoch):
                for step, data in enumerate(train_loader):  # batch = tuple(t.to(args.device) for t in batch)
                    ...
                    x, y_gts = data[0].to(args.device), data[1].to(args.device)
        ...
        # then evaluate
        test_ds = DataSetCatCon_mlp(x_test_all, y_test_all, args.cat_idxs, args.dtask, continuous_mean_std)
...
def main():
    ...
    parser.add_argument("--net_name", type=str, default="mlp",
                        help="Basic Name of this run with detailed network-architecture selection. ")
    parser.add_argument("--FL_platform", type=str, default="MLP",
                        choices=["Swin-FedAVG", "ViT-FedAVG", "Swin-FedAVG", "EfficientNet-FedAVG", "ResNet-FedAVG",
                                 "saint-FedAVG_ori", "MLP"], help="Choose of different FL platform. ")
