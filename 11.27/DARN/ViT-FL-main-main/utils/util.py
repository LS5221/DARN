...
def classification_scores(model, dloader, device, task, attentiontype):
    ...
    with torch.no_grad():
        for i, data in enumerate(dloader, 0):
            x_categ, x_cont, y_gts, cat_mask, con_mask, cat_ips, con_ips, rowips = data[0].to(device), data[1].to(
                device), data[2].to(device), data[3].to(device), data[4].to(device), data[5].to(device), data[6].to(
                device), data[7].to(device)
            _, x_categ_enc, x_cont_enc = embed_data_mask(x_categ, x_cont, cat_mask, con_mask, model)
    ...
    if task == 'binary':
        ...
        auc = roc_auc_score(y_score=prob.cpu(), y_true=y_test.cpu(), multi_class='ovo')
    return acc.cpu().numpy(), auc
def classification_scores_mlp(model, dloader, device, task, attentiontype):
    model.eval()
    m = nn.Softmax(dim=1)
    y_test = torch.empty(0).to(device)
    y_pred = torch.empty(0).to(device)
    ...
    with torch.no_grad():
        for i, data in enumerate(dloader, 0):
            x_cont, y_gts = data[0].to(device), data[1].to(device)
...
def compute_weight(mask_dic, ips_dic):
    ...
    tensor_values = list(ips_dic.values())
    ...
    for i in range(num_tensors):
        for j in range(i, num_tensors):
            if i != j:
                # 计算余弦相似度
                cosine_similarity_matrix[i, j] = cosine_similarity_matrix[j, i] = F.cosine_similarity(
                    tensor_values[i].flatten(),
                    tensor_values[j].flatten(), dim=0)

                # 计算标准化后的皮尔逊相关系数
                mean_i = torch.mean(tensor_values[i], dim=0)
                mean_j = torch.mean(tensor_values[j], dim=0)
                std_i = torch.std(tensor_values[i], dim=0)
                std_j = torch.std(tensor_values[j], dim=0)
                tensor_i_normalized = (tensor_values[i] - mean_i) / (std_i + 1e-8)
                tensor_j_normalized = (tensor_values[j] - mean_j) / (std_j + 1e-8)
                pearson_correlation_matrix[j, i] = pearson_correlation_matrix[i, j] = F.cosine_similarity(
                    tensor_i_normalized.flatten(),
                    tensor_j_normalized.flatten(), dim=0)
    return cosine_similarity_matrix, pearson_correlation_matrix
