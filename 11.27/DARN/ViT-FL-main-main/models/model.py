...
class Attention_withoutmask_ips(nn.Module):
    def __init__(
        self,
        dim,
        heads = 8,
    ...
    def forward(self, x, rowips=None):
        h = self.heads
        q, k, v = self.to_qkv(x).chunk(3, dim=-1)
        q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h = h), (q, k, v))
        sim = einsum('b h i d, b h j d -> b h i j', q, k) * self.scale
        rowips = rowips.unsqueeze(0)
        # print(sim.shape, rowips.shape)
        # assert sim == 0
        sim = sim * rowips
...
class RowColTransformer(nn.Module):
        ...
        for _ in range(depth):
            if self.style == 'colrow':
                self.layers.append(nn.ModuleList([
                    ...
                    PreNorm(dim*nfeats, Residual(Attention_withoutmask_ips(dim*nfeats, heads = heads, dim_head = 64, dropout = attn_dropout))),
    ...
    def forward(self, x, x_cont=None, cont_mask=None, cat_mask=None, cat_ips=None, con_ips=None, rowips=None):
        ...
        if self.style == 'colrow':
            for attn1, ff1, attn2, ff2 in self.layers:
                ...
                # print("rowips", rowips.shape, attn2)
                x = attn2(x, rowips=rowips)
                x = ff2(x)
